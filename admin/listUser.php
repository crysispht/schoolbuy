<?php
    ...
    checkLogined();
    connect();
    $sql = "select uid,uaccount,uemail,udelete from users";
    $rows = fetchAll($sql);
    if (!$rows) {
        ...
        <?php foreach ($rows as $row): ?>
            <tr>
                ...
                <td><input type="checkbox" id="c1" class="check"><label for="c1"
                                                                        class="label"><?php echo $row['uid']; ?></label>
                </td>
                <td><?php echo $row['uaccount']; ?></td>
                <td><?php echo $row['uemail']; ?></td>
                <td>
                    <?php
                        echo $row['udelete'] == 1 ? "未激活" : "激活";
                    ?>
                </td>
                <td align="center"><input type="button" value="修改" class="btn"
                                          onclick="editUser(<?php echo $row['uid']; ?>)"><input type="button" value="删除"
                                                                                               class="btn"
                                                                                               onclick="delUser(<?php echo $row['uid']; ?>)">
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
