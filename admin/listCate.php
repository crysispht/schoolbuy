<?php
    ...
    $sql = "select cateid,cName from cates order by cateid asc limit {$offset},{$pageSize}";
    $rows = fetchAll($sql);
    if (!$rows) {
        ...
        <?php foreach ($rows as $row): ?>
            <tr>
                ...
                <td><input type="checkbox" id="c1" class="check"><label for="c1"
                                                                        class="label"><?php echo $row['cateid']; ?></label>
                </td>
                <td><?php echo $row['cName']; ?></td>
                <td align="center"><input type="button" value="修改" class="btn"
                                          onclick="editCate(<?php echo $row['cateid']; ?>)"><input type="button" value="删除"
                                                                                               class="btn"
                                                                                               onclick="delCate(<?php echo $row['cateid']; ?>)">
                </td>
            </tr>
        <?php endforeach; ?>
        <?php if ($totalRows > $pageSize): ?>
            <tr>
                <td colspan="4"><?php echo showPage3($page, $totalPage); ?></td>
            </tr>
        <?php endif; ?>
        </tbody>
    </table>
