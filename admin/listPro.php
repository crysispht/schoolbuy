<?php
    ...
    //得到数据库中所有商品
    $sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from elecpro as p join cates c on p.cId=c.cateid {$where}  ";
    connect();
    ...
    $sql = "select p.id,p.pName,p.pSn,p.pNum,p.mPrice,p.iPrice,p.pDesc,p.pubTime,p.isShow,p.isHot,c.cName from elecpro as p join cates c on p.cId=c.cateid {$where} {$orderBy} limit {$offset},{$pageSize}";
    $rows = fetchAll($sql);
?>
<!doctype html>
<html>
...
<div class="details">
    ...
    <table class="table" cellspacing="0" cellpadding="0">
                ...
                <td align="center">
                    ...
                    <div id="showDetail<?php echo $row['id']; ?>" style="display:none;">
                        <table class="table" cellspacing="0" cellpadding="0">
                            ...
                            <tr>
                                <td width="20%" align="right">折扣价格</td>
                                ...
                                <td>
                                    <?php
                                        ...
                                        foreach ($proImgs as $img):
                                            ?>
                                            <img width="100" height="100" src="../uploads/elecpro/<?php echo explode(',',$img['albumPath'])[0]; ?>"
                                                 alt=""/> &nbsp;&nbsp;
                                        <?php endforeach; ?>
                                </td>
                            </tr>
        ...
        <?php if ($totalRows > $pageSize): ?>
            <tr>
                <td colspan="7"><?php echo showPage3($page, $totalPage, "keywords={$keywords}&order={$order}"); ?></td>
            </tr>
        <?php endif; ?>
        </tbody>
    </table>
